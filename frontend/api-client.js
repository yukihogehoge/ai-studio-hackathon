...
class APIClient {
  ...
  async searchTouristSpots(keyword) {
    try {
      const response = await fetch(
        `${this.baseURL}/spots/search?keyword=${encodeURIComponent(keyword)}`
      );
  ...
  async searchEvents(keyword) {
    try {
      const response = await fetch(
        `${this.baseURL}/events/search?q=${encodeURIComponent(keyword)}`
      );
      if (!response.ok) throw new Error("検索に失敗しました");
      return await response.json();
    } catch (error) {
  ...
  async getTopSpots(limit = 5) {
    try {
      const response = await fetch(
        `${this.baseURL}/stats/top-spots?limit=${limit}`
      );
  ...
  async getNearbySpots(lat, lon) {
    try {
      const response = await fetch(
        `${this.baseURL}/spots/nearby?lat=${lat}&lon=${lon}`
      );
