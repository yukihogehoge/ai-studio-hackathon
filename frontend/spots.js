...
async function handleLogin() {
  ...
  try {
    ...
    if (result.success) {
      ...
      alert(
        "ログインに失敗しました: " +
          (result.message || "ユーザーIDまたはパスワードが正しくありません")
      );
    }
  } catch (error) {
  ...
  try {
    ...
    spots.forEach((spot) => {
      ...
      if (spot.address.includes("前橋") || spot.address.includes("赤城")) {
        ...
        spot.address.includes("高崎") ||
        spot.address.includes("富岡")
      ) {
        area = "takasaki";
      } else if (
        spot.address.includes("草津") ||
        spot.address.includes("四万")
      ) {
        area = "kusatsu";
      } else if (
        spot.address.includes("水上") ||
        spot.address.includes("みなかみ") ||
        spot.address.includes("利根郡") ||
        spot.spot_name.includes("尾瀬") ||
        spot.spot_name.includes("谷川")
      ) {
        area = "minakami";
      } else if (
        spot.address.includes("伊香保") ||
        spot.address.includes("渋川") ||
        spot.spot_name.includes("榛名")
      ) {
        area = "ikaho";
      }
      ...
      const ratingText =
        spot.review_count > 0
          ? `${starsHtml} ${spot.avg_rating.toFixed(1)} (${
              spot.review_count
            }件)`
          : "評価なし";
      ...
      spotElement.innerHTML = `
                ...
                <div class="spot-info">
                    ...
                    <h3><a href="spot-detail.html?id=${spot.spot_id}">${
        spot.spot_name
      }</a></h3>
    ...
    alert(
      "観光地の読み込みに失敗しました。サーバーが起動していることを確認してください。"
    );
  }
}
...
function loadRanking(spots) {
  ...
  sortedSpots.slice(0, 10).forEach((spot, index) => {
    ...
    const ratingDisplay =
      spot.review_count > 0
        ? `<div style="color: #ffd700; font-size: 0.9rem;">${starsHtml} ${spot.avg_rating.toFixed(
            1
          )}</div>`
        : '<div style="color: #999; font-size: 0.9rem;">評価なし</div>';

    rankItem.innerHTML = `
            <span class="ranking-number">${index + 1}</span>
            <span class="ranking-name">
                <a href="spot-detail.html?id=${spot.spot_id}">${
      spot.spot_name
    }</a>
                ${ratingDisplay}
            </span>
...
function displaySearchResults(results, keyword) {
  ...
  if (results.length === 0) {
    spotsGrid.innerHTML =
      '<p style="text-align: center; color: #999; padding: 40px;">検索結果が見つかりませんでした</p>';
    return;
  }
  ...
  results.forEach((spot) => {
    ...
    const ratingText =
      spot.review_count > 0
        ? `${starsHtml} ${spot.avg_rating.toFixed(1)} (${spot.review_count}件)`
        : "評価なし";
...
function filterByArea(area, clickedButton) {
  ...
  console.log(
    `フィルター: ${area} - ${
      document.querySelectorAll(`.spot-item[data-area="${area}"]`).length
    }件表示`
  );
}
