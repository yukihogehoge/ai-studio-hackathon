  ...
  try {
    ...
    if (spot) {
      ...
      document.getElementById(
        "spotDescription"
      ).innerHTML = `<p>${spot.description}</p>`;
      ...
      const ratingText =
        spot.review_count > 0
          ? `${starsHtml} ${Number(spot.avg_rating).toFixed(1)} (${
              spot.review_count
            }件のレビュー)`
          : "評価なし";
      ...
      const creditHtml = hasCredit
        ? `<div style="margin-top: 8px; font-size: 0.85em; color: #666; text-align: right; display: block; clear: both;">${
            imageCredits[spot.spot_id]
          }</div>`
        : "";
      ...
      // パンくずリストの観光地名も更新
      const breadcrumbSpot = document.querySelector(
...
async function loadReviews() {
  ...
  try {
    ...
    if (reviews.length === 0) {
      reviewsList.innerHTML =
        '<p style="text-align: center; color: #999;">まだレビューがありません。最初のレビューを投稿してください！</p>';
      return;
    }
    ...
    reviews.forEach((review) => {
      ...
      const dateStr = `${reviewDate.getFullYear()}年${
        reviewDate.getMonth() + 1
      }月${reviewDate.getDate()}日`;
      ...
      const deleteButtonHtml =
        currentUser && Number(review.user_id) === Number(currentUser.user_id)
...
async function submitReview(event) {
  ...
  try {
    ...
    const userReview = existingReviews.find(
      (r) => Number(r.user_id) === Number(currentUser.user_id)
    );
    if (userReview) {
      alert(
